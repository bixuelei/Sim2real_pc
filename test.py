...
def train(args, io):
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        #     loss = criterion(seg_pred.view(-1, NUM_CLASS), target.view(-1,1).squeeze(),weights,using_weight=args.use_weigth)     #a scalar
        ...
        with torch.no_grad():         
            ...
            for i,(points,seg) in tqdm(enumerate(test_loader),total=len(test_loader),smoothing=0.9):
                ...
                # seg_pred=torch.matmul(seg_pred,weights)
                seg_pred = seg_pred.contiguous().view(-1, NUM_CLASS)   # (batch_size*num_points , num_class)
                pred_choice = seg_pred.cpu().data.max(1)[1].numpy()  #array(batch_size*num_points)
            ...
            for l in range(NUM_CLASS):
                iou_per_class_str += 'class %s percentage: %.3f, loss_weight: %.3f, IoU: %.3f \n' % (
...
def test(args, io):
    ...
    with torch.no_grad():         
        ...
        iou_per_class_str = '------- IoU --------\n'
        for l in range(NUM_CLASS):
            iou_per_class_str += 'class %s percentage: %.3f, IoU: %.3f \n' % (
...
if __name__ == "__main__":
    ...
    parser.add_argument('--change', type=str, default='allaround_STN_16_2048_150_one_more_edgeconv_cos_e2_reversed', metavar='N',
                        help='experiment version to record reslut')
    ...
    parser.add_argument('--bolt_weight', type=float, default=1, 
